...
class LatchApp(LatchAuth):
    ...
    def lock(self, account_id, operation_id=None, instance_id=None):
        if operation_id == None:
	    if instance_id == None:
                return self._http("POST", self.API_LOCK_URL + "/" + account_id)
	    else:
	        return self._http("POST", self.API_LOCK_URL + "/" + account_id + "/i/" + instance_id)
        else:
	    if instance_id == None:
                return self._http("POST", self.API_LOCK_URL + "/" + account_id + "/op/" + operation_id)
	    else:
		return self._http("POST", self.API_LOCK_URL + "/" + account_id + "/op/" + operation_id + "/i/" + instance_id)        

    def unlock(self, account_id, operation_id=None, instance_id=None):
	if operation_id == None:
	    if instance_id == None:
                return self._http("POST", self.API_UNLOCK_URL + "/" + account_id)
	    else:
	        return self._http("POST", self.API_UNLOCK_URL + "/" + account_id + "/i/" + instance_id)
        else:
	    if instance_id == None:
                return self._http("POST", self.API_UNLOCK_URL + "/" + account_id + "/op/" + operation_id)
	    else:
		return self._http("POST", self.API_UNLOCK_URL + "/" + account_id + "/op/" + operation_id + "/i/" + instance_id)
